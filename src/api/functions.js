...
export const seedDummyProjects = async ({ companyId }) => {
  ...
  try {
    ...
    const result = await supabaseFunctions.invoke('seedDummyProjects', {
      body: { company_id: companyId }
    });
    
    if (result.error) {
      console.error('[seedDummyProjects] Edge function error:', result.error);
      return [];
    }
    
    console.log('[seedDummyProjects] Result:', result.data);
...
export const deleteDummyProjects = async ({ companyId }) => {
  ...
  try {
    // Use Edge Function to bypass RLS
    const result = await supabaseFunctions.invoke('deleteDummyProjects', {
      body: { company_id: companyId }
    });
    
    if (result.error) {
      console.error('[deleteDummyProjects] Edge function error:', result.error);
    } else {
      console.log('[deleteDummyProjects] Result:', result.data);
    }
  } catch (error) {
